...
class TririgaNLPRouter:
    def __init__(self, diagnostic_engine, ssh_host=None, ssh_user=None, ssh_log_path=None):
    ...
    def _fetch_remote_log(self, lines_to_read, show_workflow_note=False):
        ...
        try:
            ...
            return "ERROR: The 'paramiko' library is required. Please run: pip install paramiko"
    ...
    def process_query(self, user_query):
        ...
        if any(w in query for w in ["why did it fail", "check the log", "scan log", "what just failed", "read log"]):
            if self.ssh_host and self.ssh_user and self.ssh_log_path:
                return self.scan_live_log_ssh(user_query=user_query)
            return self._wrap_ascii(user_query, "I cannot scan the live logs because the SSH configuration is missing in the engine.")

        elif any(w in query for w in ["another workflow", "trace ad hoc", "ad hoc trace", "ad-hoc trace", "external workflow", "not in omp"]):
            if self.ssh_host and self.ssh_user and self.ssh_log_path:
                return self.trace_ad_hoc_live_execution_ssh(user_query=user_query)
            return self._wrap_ascii(user_query, "I cannot trace live logs because the SSH configuration is missing in the engine.")

        elif any(w in query for w in ["trace live", "how did it execute", "live execution", "trace execution"]):
            if self.ssh_host and self.ssh_user and self.ssh_log_path:
                return self.trace_live_execution_ssh(user_query=user_query)
            return self._wrap_ascii(user_query, "I cannot trace live logs because the SSH configuration is missing in the engine.")
            ...
            return self._wrap_ascii(user_query, "Could not parse the condition. Please specify 'when [field] is [value]'.")
            ...
            return self._wrap_ascii(user_query, "Could not identify a valid TRIRIGA string to search for.")
            ...
            return self._wrap_ascii(user_query, "Could not identify a Task ID. Please specify the task number.")
            ...
            return self._wrap_ascii(user_query, self._find_orphans())
       
        match_path = re.search(r"path.*from\s+['\"]?(.*?)['\"]?\s+to\s+['\"]?(.*?)['\"]?", query)
        if match_path:
            return self._wrap_ascii(user_query, self._trace_path(match_path.group(1), match_path.group(2)))
    ...
    def _explain_purpose_with_paths(self, user_query):
        ...
        if not wf_name:
            if len(self.engine.loaded_workflow_names) == 1:
                ...
                return self._wrap_ascii(user_query, "Multiple workflows are loaded. Please run 'trace live execution' on one first so I know which context to explicitly explain.")
        
        base_summary = self._explain_purpose(wf_name)
        all_paths = self._generate_all_paths(wf_name)
        
        if not all_paths: return self._wrap_ascii(user_query, base_summary)
        
        total_paths = len(all_paths)
        current_idx = 1
        path_text = self._format_path_narrative(all_paths[0], 1, total_paths)
        
        if total_paths == 1:
            return self._wrap_ascii(user_query, base_summary + "\n\n" + path_text)
        ...
        while current_idx < total_paths:
            ...
            if ans.strip().lower() in ['y', 'yes']:
                current_idx += 1
                path_text = self._format_path_narrative(all_paths[current_idx-1], current_idx, total_paths)
                if current_idx == total_paths:
                    return self._wrap_ascii(f"[Interactive] Exploring Path {current_idx}", path_text + "\n\n[!] All logic paths successfully explored.")
                else:
                    print(self._wrap_ascii(f"[Interactive] Exploring Path {current_idx}", path_text))
            elif ans.strip().lower() in ['n', 'no']:
                return self._wrap_ascii(f"[Interactive] Declined remaining paths", "Path exploration successfully closed.")
            else:
    ...
    def _format_path_narrative(self, path, current_idx, total_paths):
    ...
    def scan_live_log_ssh(self, lines_to_read=5000, user_query=""):
        log_data = self._fetch_remote_log(lines_to_read, show_workflow_note=False)
        if isinstance(log_data, str) and log_data.startswith("ERROR:"): return self._wrap_ascii(user_query, log_data)
        ...
        if not relevant_errors:
            return self._wrap_ascii(user_query, "Log scan complete. Good news: No recent errors in the log match any of the Task IDs in your loaded workflow. (Other developers' errors were successfully ignored).")

        latest_task_id, latest_error, wf_name = relevant_errors[-1]
        self.current_context_wf = wf_name 
        return self._wrap_ascii(user_query, self._synthesize_log_failure(latest_task_id, latest_error, wf_name))
    ...
    def trace_ad_hoc_live_execution_ssh(self, lines_to_read=15000, user_query=""):
        ...
        if not target_wf_name: return self._wrap_ascii(user_query, "Ad-hoc trace cancelled. No workflow name provided.")

        log_lines = self._fetch_remote_log(lines_to_read, show_workflow_note=True)
        if isinstance(log_lines, str) and log_lines.startswith("ERROR:"): return self._wrap_ascii(user_query, log_lines)
        ...
        if not relevant_wfiids:
            return self._wrap_ascii(user_query, f"Ad-Hoc Profiler complete. No executions for '{target_wf_name}' were found in the recent log.\nEnsure Workflow Logging is enabled, check your spelling, trigger the workflow, and try again.")
        ...
        print(self._wrap_ascii(user_query, "\n".join(output)))
        ...
        while True:
            ...
            if ans.strip().lower() in ['y', 'yes']:
                ...
                return self._wrap_ascii("[Interactive] No", f"Resetting to main menu.\n{instructions}")
            else:
                print("\n[!] Please answer Yes or No.")
    ...
    def trace_live_execution_ssh(self, lines_to_read=15000, user_query=""):
        ...
        if not relevant_wfiids:
            return self._wrap_ascii(user_query, "Execution Profiler complete. No tasks from the targeted OM Package workflow were found in the recent log.\nEnsure Workflow Logging is enabled, trigger your workflow in the front end, and try again.")
        ...
        return self._wrap_ascii(user_query, "\n".join(output))
    ...
    def _trace_condition(self, field, value, user_query):
        ...
        if not output: return self._wrap_ascii(user_query, f"Could not find any Decision Gate evaluating the condition '{value}' across active contexts.")
        return self._wrap_ascii(user_query, f"Forward Execution Trace:\n\n" + "\n\n".join(output))
    ...
    def _find_references(self, search_term, user_query):
        ...
        if not output: return self._wrap_ascii(user_query, f"No tasks in active workflow context currently reference or touch '{search_term}'.")
        return self._wrap_ascii(user_query, f"Reverse Search Results for '{search_term}':\n" + "\n".join(output))
    ...
    def _analyze_failure(self, task_identifier, user_query):
        ...
        if not found_nodes: return self._wrap_ascii(user_query, f"Error: Could not find task '{task_identifier}' in active workflow context.")
    ...
    def _explain_task_logic(self, task_identifier, user_query):
        ...
        if len(spec_ids_to_query) > 1:
            print(self._wrap_ascii(user_query, output_str))
            ...
            while True:
                ...
                if ans.strip().lower() in ['y', 'yes']:
                    ...
                    return self._wrap_ascii("Yes (Displaying remaining records)", "\n".join(remainder_output))
                elif ans.strip().lower() in ['n', 'no']:
                    return self._wrap_ascii(ans, "Remaining payloads discarded.")
                else:
                    print("\n[!] Please answer Yes or No.")
        else:
            return self._wrap_ascii(user_query, output_str)
...
if __name__ == "__main__":
    from tririga_hybrid_diagnostic_engine import TririgaHybridEngine
    ...
    engine = TririgaHybridEngine(DB_USER, DB_PASS, DB_DSN)
    
    if engine.load_om_package(OM_PACKAGE_NAME):
        
        router = TririgaNLPRouter(engine, ssh_host=SSH_HOST, ssh_user=SSH_USER, ssh_log_path=SSH_LOG_PATH)
        
        print("\n=== AI Diagnostic Chat Initialized ===")
        print("Type 'exit' to quit.")
        print("Type 'scan log' or 'what just failed' to securely scan the live logs for errors.")
